namespace SolidUtilities.UnityEditorInternals
{
    ...
    {
        // TODO: figure out where we need to change type or add one more type for the filter to work properly.
        // TODO: It may be not in this method but in one of the nested methods at the end of this one.
        public static void ShowGeneric(this ObjectSelector this_, SerializedProperty property, bool allowSceneObjects)
        {
            ...
            if (requiredType != null)
                this_.m_RequiredType = typeof(ScriptableObject).IsAssignableFrom(requiredType) || typeof(MonoBehaviour).IsAssignableFrom(requiredType) ? requiredType.FullName : requiredType.Name;
